// agent/ast/__tests__/helpers.ts
...
export const FIXTURES_DIR = path.join(currentDir, '../__fixtures__')
...
// Minimal structural type for node:assert/strict — only what assertGolden uses.
type AssertLike = { strictEqual(actual: unknown, expected: unknown, message?: string): void }
...
export function assertGolden(assert: AssertLike, testName: string, actual: string): void {
